...
def get_default_parameters() -> dict[str, object]:
    """Get default parameters based on LyricsExtract defaults and PARAMETER_GRID."""
    ...
    # Use actual defaults if they're in the grid, otherwise use closest or first value
    params = {}
    for name in PARAMETER_COLUMNS:
        ...
        if default_val in grid_values:
            ...
            # Use closest value or first value
...
def parameter_combinations() -> Iterable[dict[str, object]]:
    # First yield the default parameters as baseline
    default_params = get_default_parameters()
    yield default_params

    # Then yield all other combinations, skipping the default
...
def load_semantic_model(args: argparse.Namespace):
    from sentence_transformers import SentenceTransformer
...
def build_evaluation_fn(args: argparse.Namespace, semantic_model):
    # Keep heavyweight model objects per worker thread to avoid repeated reloads.
    worker_local = threading.local()
    device_reported = threading.Event()
    single_worker_resources: dict[str, object] = {}

    if args.workers == 1:
        # In single-worker mode there is no cross-thread sharing benefit,
        # so initialize once and avoid shared-model coordination overhead.
        from g2p_en import G2p
